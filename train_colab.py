...
#        Right-click "dataset" folder → Send to → Compressed (zip)
...
# (These are already in Colab but pinning versions is good practice)
# !pip install timm -q
...
# Update this path to wherever you uploaded dataset.zip in Drive
...
CFG = {
    ...
    "weight_decay":    1e-4,
    "label_smoothing": 0.1,
    "checkpoint_path": "/content/drive/MyDrive/ultrasound_efficientnet_b3.pth",
}

# Quality label → operator guidance text
GUIDANCE = {
...
# ── CELL 4: Data loaders ─────────────────────────────────────
# EfficientNet-B3 was pretrained with these ImageNet stats
MEAN = [0.485, 0.456, 0.406]
STD  = [0.229, 0.224, 0.225]

# Training: mild geometric augmentation only.
# Do NOT add blur/brightness jitter — those are already your label-defining
# corruptions and adding more would muddy class boundaries.
...
CLASS_NAMES = train_ds.classes   # alphabetical: angled, blurry, good, ...
...
def unfreeze_last_blocks(model: nn.Module, n_blocks: int = 3):
    """Unfreeze last n feature blocks + classifier for fine-tuning."""
    # EfficientNet-B3 has 7 feature blocks (features[1] through features[7])
    total_blocks = 8   # features[0..7]
...
# ── CELL 8: Phase 2 — fine-tune last blocks ──────────────────
...
def predict(image_path: str, model: nn.Module) -> dict:
    """
    Given a path to an ultrasound image, returns:
      - quality_label  : predicted class name
      - confidence     : softmax probability
      - guidance_text  : instruction for the operator
...
# --- Demo: pick a random test image and run inference ---
...
plt.title(f"Predicted: {result['quality_label']} ({result['confidence']:.1%})\n"
          f"{result['guidance_text']}", wrap=True, fontsize=9)
plt.axis("off")
plt.tight_layout()
plt.show()
